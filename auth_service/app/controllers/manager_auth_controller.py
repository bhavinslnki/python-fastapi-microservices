...
async def register_manager(email: str, password: str,first_name:str,last_name:str,phone_number: str , db):
    try:
        ...
        db.refresh(new_user)
        get_user_role = db.query(Role).filter(Role.role_name =="manager").first()
...
async def login_manager(email: str, password: str, db):
    try:
