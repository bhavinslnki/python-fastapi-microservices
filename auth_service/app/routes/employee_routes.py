...
from app.controllers.employee_auth_controller import register_employee,login_employee,send_otp,verify_otp
...
async def register(request:CommonRegisterRequest,db:Session=Depends(get_db)):
    return await register_employee(request.email,request.password,request.first_name,request.last_name,request.phone_number,db)

@router.post('/login')
async def login(request:CommonLoginRequest,db:Session=Depends(get_db)):
    return await login_employee(request.email,request.password,db)
